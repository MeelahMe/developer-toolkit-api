...
def test_generate_password_default(client, auth_headers):
    """
    Test generating a password using default parameters.
    Should be 12 characters long.
    """
    response = client.get("/tools/password/generate", headers=auth_headers)
    assert response.status_code == 200
    assert "password" in response.json()
    assert len(response.json()["password"]) == 12


def test_generate_password_custom_length(client, auth_headers):
    """
    Test that the password generator respects custom length.
    """
    response = client.get("/tools/password/generate?length=20", headers=auth_headers)
    assert response.status_code == 200
    assert len(response.json()["password"]) == 20


def test_generate_password_without_symbols(client, auth_headers):
    """
    Test password generation excluding symbols.
    """
    response = client.get(
        "/tools/password/generate?include_symbols=false", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_generate_password_with_all_sets_false(client, auth_headers):
    """
