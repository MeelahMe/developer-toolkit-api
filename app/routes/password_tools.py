...
class PasswordResponse(BaseModel):
    password: str = Field(
        example="a9B!7xTq@1l#", description="A randomly generated secure password"
    )
...
def generate_password(
    ...
    include_symbols: bool = Query(
        True, description="Include special characters (!@#...)"
    ),
    include_numbers: bool = Query(True, description="Include digits (0-9)"),
    include_uppercase: bool = Query(
        True, description="Include uppercase letters (A-Z)"
    ),
    include_lowercase: bool = Query(
        True, description="Include lowercase letters (a-z)"
    ),
):
    """
